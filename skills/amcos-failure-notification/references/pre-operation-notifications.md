...
**Verify**: confirm message delivery via the `amp-send` CLI output.
...
Send a timeout notice via the `amp-send` CLI:
- **Recipient**: the target agent session name
...
Send a pre-operation notification via the `amp-send` CLI:
