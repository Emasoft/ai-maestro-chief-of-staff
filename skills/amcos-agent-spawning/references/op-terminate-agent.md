...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority high`) a status request:
...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority high`) to request a state dump:
- **Recipient**: the target agent session name
...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type notification --priority urgent`) a termination notice:
- **Recipient**: the target agent session name
...
`amcos_team_registry.py` has no `log` subcommand — note the termination event by
sending a message via the `amp-send` CLI instead.
...
1. Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority high`) a pre-termination status check:
...
3. Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type notification --priority urgent`) a termination warning:
   - **Recipient**: `dev-backend-alice`
   - **Subject**: `Termination Notice`
   - **Priority**: `urgent`
