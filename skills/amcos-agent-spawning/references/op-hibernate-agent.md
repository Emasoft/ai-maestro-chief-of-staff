...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority normal`) an idle check:
...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type notification --priority high`) a hibernation notice:
- **Recipient**: the target agent session name
...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority high`) to request state capture:
- **Recipient**: the target agent session name
...
1. Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority normal`) to check idle status:
...
3. Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type notification --priority high`) hibernation warning:
   - **Recipient**: `dev-frontend-bob`
   - **Subject**: `Hibernation Notice`
   - **Priority**: `high`
