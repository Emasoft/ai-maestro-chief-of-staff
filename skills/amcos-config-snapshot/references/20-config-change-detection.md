...
Config change detection is the process of identifying when central configuration files (in `design/config/` - OPTIONAL: If EOA (AI Maestro Orchestrator Agent) plugin is installed) have been modified since the session config snapshot was created. This allows the agent to respond appropriately to config updates from the orchestrator.
