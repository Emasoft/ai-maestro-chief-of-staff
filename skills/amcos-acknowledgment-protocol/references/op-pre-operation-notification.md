...
- [ ] Sent notification with high priority via `amp-send`
...
Send via the `amp-send` CLI: `amp-send code-impl-auth "Proceeding Without Acknowledgment" "<message>" --type notification --priority high`.
- **Recipient**: `code-impl-auth`
...
For each agent in the team (`code-impl-auth`, `test-engineer-01`, `docs-writer`), send via the `amp-send` CLI: `amp-send <agent> "System Maintenance in 5 Minutes" "<message>" --type notification --priority high`.
