...
- The `amp-*` CLIs (`amp-send`, `amp-inbox`, `amp-read`, `amp-reply`) are available
...
At each reminder interval, if no response, send via the `amp-send` CLI a reminder:
- **Recipient**: the target agent session name
...
Use `amp-inbox` to check for unread messages from the target agent. Look for responses containing:
...
**If timeout occurs:**
1. Send via the `amp-send` CLI a final timeout notice:
   - **Recipient**: the target agent session name
...
- [ ] Sent acknowledgment request with clear instructions via `amp-send`
...
Send via the `amp-send` CLI (2 minute timeout):
...
Send via the `amp-send` CLI (30 second timeout):
