...
**Purpose:** Deliver the notification using the `amp-send` CLI.

Send via the `amp-send` CLI: `amp-send <recipient> "<subject>" "<message>" --type notification --priority high`.
...
Send via the `amp-send` CLI: `amp-send <recipient> "Proceeding Without Acknowledgment" "<message>" --type notification --priority high`.
- **Recipient**: the target agent session name
...
Send via the `amp-send` CLI: `amp-send code-impl-auth "Skill Installation Pending" "<message>" --type notification --priority high`.
...
Send via the `amp-send` CLI: `amp-send test-engineer-01 "Plugin Installation Pending" "<message>" --type notification --priority high`.
...
For each agent in the team (`code-impl-auth`, `test-engineer-01`, `docs-writer`, `devops-ci`), send via the `amp-send` CLI: `amp-send <agent> "System Maintenance Scheduled" "<message>" --type notification --priority high`.
