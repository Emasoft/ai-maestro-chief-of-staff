...
jq --arg task "task-001" '.tasks[] | select(.task_id == $task)' \
  $AGENT_DIR/db/agent-health/task-tracking.json
```
...
' $AGENT_DIR/db/agent-health/task-tracking.json > temp.json && mv temp.json $AGENT_DIR/db/agent-health/task-tracking.json
```
