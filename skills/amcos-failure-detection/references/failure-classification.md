...
echo "{\"timestamp\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\", \"agent\": \"AGENT_NAME\", \"type\": \"transient\", \"symptoms\": \"missed_heartbeat\", \"resolved\": false}" >> $AGENT_DIR/db/agent-health/incident-log.jsonl
...
```
$AGENT_DIR/db/agent-health/incident-log.jsonl
```
