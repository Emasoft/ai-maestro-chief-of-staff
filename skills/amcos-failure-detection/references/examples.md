...
- **Step 6**: Report recovery to AMAMA via the `amp-send` CLI: `amp-send <recipient> "<subject>" "<message>" [--type T] [--priority P]`:
...
- **Step 3**: Request replacement approval via the `amp-send` CLI:
  - **Recipient**: `amama-assistant-manager`
...
- **Step 5**: After approval, notify the orchestrator via the `amp-send` CLI:
...
- **Step 3**: Initiate emergency handoff via the `amp-send` CLI:
  - **Recipient**: `amoa-orchestrator`
...
- **Step 4**: Notify the manager via the `amp-send` CLI:
  - **Recipient**: `amama-assistant-manager`
...
Send a heartbeat via the `amp-send` CLI:
...
Request a graceful restart via the `amp-send` CLI:
- **Recipient**: the target agent session name
...
Request replacement approval via the `amp-send` CLI:
- **Recipient**: `amama-assistant-manager`
...
Request emergency handoff via the `amp-send` CLI:
- **Recipient**: `amoa-orchestrator`
