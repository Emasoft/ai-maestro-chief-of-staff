...
- Session Memory Components - The three files (activeContext.md, patterns.md, progress.md)
- Steps - Step-by-step initialization procedure

#### Update Active Context ([references/op-update-active-context.md](references/op-update-active-context.md))
Runbook for updating the activeContext.md file when the agent's current focus, decisions, or session state changes.
- Update Triggers - When to perform context updates
- Steps - Identify change, open file, update section, write immediately, update timestamp
- Update Patterns - Task switch, decision recording, and other update patterns
...
- Steps - Step-by-step pattern recording procedure

#### Update Task Progress ([references/op-update-task-progress.md](references/op-update-task-progress.md))
Runbook for updating task status, documenting blockers, and managing dependencies in progress.md when task state changes.
- Task States - Available task states
- Steps - Identify task, open progress.md, update status, document blockers, update dependencies, update timestamp
- Progress File Structure - How progress.md is organized
- Checklist - Verification steps after update

#### Recover Session After Interruption ([references/op-recover-session.md](references/op-recover-session.md))
...
- Steps - Load memory files, read work state, read task state, validate consistency, ask user to confirm resumption
- Recovery Scenarios - Clean resume, long gap recovery, corrupted memory recovery

#### Prepare for Context Compaction ([references/op-prepare-context-compaction.md](references/op-prepare-context-compaction.md))
Runbook for safely preparing all session memory for context compaction to prevent data loss during context window reset.
- Context Compaction Risks - What can go wrong during compaction
- Steps - Check context usage, update memory files, run validation, write to disk, create backup, confirm ready
- Pre-Compaction Checklist - Preparation, update, backup, validation, and final phases

#### Capture Config Snapshot at Session Start ([references/op-capture-config-snapshot.md](references/op-capture-config-snapshot.md))
...
- Steps - Identify config files, create snapshot header, copy config content, calculate file hashes, save snapshot, record in activeContext.md
- Snapshot Structure - How the config snapshot file is organized
- Checklist - Verification steps after snapshot creation

#### Detect Config Changes During Session ([references/op-detect-config-changes.md](references/op-detect-config-changes.md))
...
- Steps - Read current configs, compare timestamps, perform content comparison, identify changed sections, log in activeContext.md, trigger conflict resolution
- Change Classification - How to classify detected changes
- Detection Methods - Timestamp-based (fast) and hash-based (accurate)
- Periodic Check Schedule - When and how often to check

#### Handle Config Version Conflicts ([references/op-handle-config-conflicts.md](references/op-handle-config-conflicts.md))
...
- Conflict Types - Type A through Type D with resolution strategies
- Steps - Classify conflict, resolve per type, update records
- Decision Tree - Flowchart for choosing the right resolution strategy
- Checklist - Verification steps for all conflict types
...
- Example 5: Recording a Discovered Pattern - Capturing a recurring error-handling pattern into patterns.md
...
For essential principles and recommended reading order, see [references/00-key-takeaways-and-next-steps.md](references/00-key-takeaways-and-next-steps.md):
- Key Takeaways - 9 essential principles for session memory management
- Next Steps - Recommended reading and integration order
