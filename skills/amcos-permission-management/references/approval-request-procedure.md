...
An approval request is a formal message sent from AMCOS (Chief of Staff) to AMAMA (Assistant Manager) asking for authorization to perform a privileged operation. The request contains all information AMAMA needs to make an informed decision on behalf of the user.
...
2. AMCOS composes and sends an approval request to AMAMA
3. AMAMA receives the request and presents it to the user
4. The user makes a decision (approve, reject, or modify)
5. AMAMA sends the decision back to AMCOS
...
- The user requests agent termination via AMAMA
...
- User requests agent wake via AMAMA
...
- The user requests plugin installation via AMAMA
...
Follow these steps to request approval from AMAMA:
...
**Step 6:** Wait for AMAMA response with timeout handling.
...
**Cause:** AMAMA may be offline, busy, or user is unavailable.

**Resolution:**
1. Use the `ai-maestro-agents-management` skill to verify AMAMA is online
2. If AMAMA offline, wait for it to come online or notify user
3. If AMAMA online but no response, follow escalation procedure
...
**Cause:** Response from AMAMA does not match expected schema.

**Resolution:**
1. Log the raw response for debugging
2. Extract decision if present in any form
3. If completely unparseable, send clarification request to AMAMA
