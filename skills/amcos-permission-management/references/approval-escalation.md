...
Approval escalation is the process of handling situations where AMAMA (the Assistant Manager) does not respond to an approval request within the expected timeframe. AMCOS cannot wait indefinitely for approval, as this would block critical operations and reduce system effectiveness.

**Escalation purpose:**
- Remind AMAMA of pending approval requests
...
**Action:** Send reminder notification to AMAMA.
...
**Action:** Send urgent notification to AMAMA.
...
2. Send to AMAMA via AI Maestro
...
3. Send to AMAMA via AI Maestro
...
The manager (via AMAMA) may issue an autonomous operation directive that allows AMCOS to perform certain operations without pre-approval.
...
# Status: resolved, decision: approved, decided_by: amama
...
### Issue: AMAMA is offline during escalation

**Symptoms:**
- Messages fail to send
- No acknowledgment of reminders

**Cause:** AMAMA session not running or AI Maestro connectivity issue.

**Resolution:**
1. Check AMAMA status in AI Maestro registry
2. If AMAMA offline: log the condition, proceed with timeout decision
3. Queue notifications for when AMAMA comes online
...
**Cause:** AMAMA may not be surfacing urgent messages to user.

**Resolution:**
1. Verify message was delivered (check AI Maestro logs)
2. Confirm AMAMA is processing urgent priority correctly
3. Consider additional notification channels for urgent matters
4. Review AMAMA's notification handling skill
...
- AMAMA receives repeated notifications
