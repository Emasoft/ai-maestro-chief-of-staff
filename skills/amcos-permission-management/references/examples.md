...
- 3. Handling Approval Timeout - When AMCOS does not respond in time
...
**Use this example when:** You need to create a new agent instance and must request approval from AMAMA.
...
AMAMA replies with type `approval_response`, `request_id`: "spawn-req-2025-02-02-001", `decision`: "approved`, `decided_at`: timestamp, `notes`: "Approved. Proceed with spawn."
...
AMAMA replies with type `approval_response`, `request_id`: "term-req-2025-02-02-002", `decision`: "rejected", `notes`: "Hibernate instead. We may need this agent for batch 2."

### Key Points

- Use `priority: "normal"` for terminate requests as they are less urgent
- Include `final_report` summarizing agent work
- Be prepared for AMAMA to suggest alternatives (hibernate instead of terminate)
- Follow AMAMA's guidance in the response

---

## 3. Handling Approval Timeout

**Use this example when:** AMCOS does not respond to your approval request within the timeout period.
...
AMCOS must have received an autonomous directive from AMAMA:
