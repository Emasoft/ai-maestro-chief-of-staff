...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority urgent`) the replacement approval request:
...
**Verify**: confirm message delivery via `amp-send`'s delivery confirmation output.
...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority high`) to request the user to create the folder:
...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority high`) to request the user to clone the repository:
- **Recipient**: the user or admin
...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority high`) to request the user to start a new Claude Code session:
- **Recipient**: the user or admin
...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type notification --priority high`) to notify the orchestrator:
...
Once AMOA has generated the handoff document, send it to the new agent via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type notification --priority urgent`):
...
After the handoff document, send specific task assignments via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type task --priority high`):
- **Recipient**: the new agent session name
...
Send a reminder via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type notification --priority urgent`):
- **Recipient**: the new agent session name
...
After acknowledgment, verify understanding via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority normal`):
- **Recipient**: the new agent session name
...
Inform the manager that replacement is complete via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type notification --priority normal`):
- **Recipient**: `amama-assistant-manager`
...
- [ ] Replacement request sent to AMAMA via `amp-send`
...
- [ ] Replacement notification sent to AMOA via `amp-send`
...
- [ ] Handoff documentation sent to new agent via `amp-send`
...
- [ ] Manager notified of completion via `amp-send`
...
**Solution**:
1. Send a reminder message to AMOA using the `amp-send` CLI
2. If no response, escalate to manager (AMAMA)
