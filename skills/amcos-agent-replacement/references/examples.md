...
- **Step 6**: Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type notification --priority normal`) to report recovery to AMAMA:
...
- **Step 3**: Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority urgent`) to request replacement approval:
  - **Recipient**: `amama-assistant-manager`
...
- **Step 5**: After approval, send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type notification --priority high`) to notify the orchestrator:
...
- **Step 3**: Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority urgent`) to initiate emergency handoff:
  - **Recipient**: `amoa-orchestrator`
...
- **Step 4**: Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type notification --priority urgent`) to notify the manager:
  - **Recipient**: `amama-assistant-manager`
...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type notification --priority low`) to send a heartbeat:
...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority urgent`) to request a graceful restart:
- **Recipient**: the target agent session name
...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority urgent`) to request replacement approval:
- **Recipient**: `amama-assistant-manager`
...
Send via the `amp-send` CLI (`amp-send <recipient> "<subject>" "<message>" --type request --priority urgent`) to request emergency handoff:
- **Recipient**: `amoa-orchestrator`
