...
- [Steps](#steps)
  ...
  - [Step 2B: Route to AMAMA (If User Decision Needed)](#step-2b-route-to-amama-if-user-decision-needed)
...
- [Checklist: When AMAMA Returns a Resolution](#checklist-when-amama-returns-a-resolution)
- [Output](#output)
- [Related References](#related-references)

## Purpose

Determine how to handle task blocker escalations - resolve directly if within AMCOS authority, or route to AMAMA if user decision required.
...
AMCOS receives escalation from AMOA
  ...
  +-- Is it a task blocker requiring user input?
        -> YES: Route to AMAMA using blocker-escalation template
```
...
2. **Classify escalation type**
   ...
   - Task blocker (user decision needed): Route to AMAMA
...
1. Monitor for AMAMA response
2. When AMAMA responds with user's decision, proceed to Step 4

### Step 4: Route Resolution Back to AMOA

1. **Receive blocker-resolution from AMAMA**
...
- [ ] Send escalation to AMAMA via AI Maestro
- [ ] Track blocker in AMCOS records
- [ ] When AMAMA responds: route resolution back to AMOA
- [ ] Verify AMOA acknowledges receipt
- [ ] Update AMCOS records

## Checklist: When AMAMA Returns a Resolution

- [ ] Receive blocker-resolution message from AMAMA
