...
Send via the `amp-send` CLI: `amp-send <target-session> "Pre-Termination Status Check" "<message>" --type request --priority high`
...
Send via the `amp-send` CLI: `amp-send <target-session> "State Dump Request" "<message>" --type request --priority high`
- **Recipient**: the target agent session name
...
Send via the `amp-send` CLI: `amp-send <target-session> "Termination Notice" "<message>" --type notification --priority urgent`
- **Recipient**: the target agent session name
...
1. Send via the `amp-send` CLI: `amp-send dev-backend-alice "Pre-Termination Status Check" "Please confirm all tasks are complete." --type request --priority high`
