...
Send via the `amp-send` CLI: `amp-send <recipient> "<subject>" "<message>" [--type T] [--priority P]` — a health check ping:
...
FAILURE_DETECTED
    ...
    +-- TERMINAL?
            ...
            YES --> Notify affected parties:
                    ...
                    - Manager (AMAMA) for critical failure
                    |
                    +-- Check recovery policy
                        |
                        +-- Auto-replace authorized? --> Route to lifecycle-manager
                        |
                        +-- Approval required? --> Request approval from AMAMA
```
...
1. Send via the `amp-send` CLI to notify the agent of pending recovery:
...
1. Send via the `amp-send` CLI to notify the orchestrator of task orphaning risk:
...
2. Send via the `amp-send` CLI to notify the manager for critical failure:
...
**Steps:**
1. Send via the `amp-send` CLI a graceful self-restart request (it saves
...
| **assistant-manager (AMAMA)** | Escalation | Approve terminal recovery actions |

**Key integration rules:**
- Always notify AMOA when tasks are orphaned
- Always notify AMAMA for terminal failures (unless pre-authorized)
