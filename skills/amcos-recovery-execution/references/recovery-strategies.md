...
3. **Continue monitoring** - send via the `amp-send` CLI periodic health check pings at increased frequency
...
Send via the `amp-send` CLI to the user or admin agent:
- **Recipient**: the user or admin agent session name
...
Sometimes a Claude Code session goes idle due to inactivity. Sending a message may wake it. Send via the `amp-send` CLI:
...
Send via the `amp-send` CLI a replacement request:
- **Recipient**: `amama-assistant-manager` (or the manager session name)
