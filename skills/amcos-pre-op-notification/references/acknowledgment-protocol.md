...
Send via the `amp-send` CLI: `amp-send <recipient> "<subject>" "<message>" [--type T] [--priority P]`, the acknowledgment request:
...
For each reminder, send via the `amp-send` CLI a reminder message:
- **Recipient**: the target agent session name
...
The agent should reply via the `amp-reply` CLI with:
...
4. Send via the `amp-send` CLI a timeout notice:
   - **Recipient**: `code-impl-auth`
   - **Subject**: `Proceeding Without Acknowledgment`
   - **Priority**: `high`
...
4. Send via the `amp-send` CLI an extension confirmation:
   - **Recipient**: `code-impl-auth`
