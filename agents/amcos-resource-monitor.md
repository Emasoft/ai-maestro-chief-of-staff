...
- **LLM Externalizer** (`mcp__plugin_llm-externalizer_llm-externalizer__*`): Use `chat` to summarize resource usage reports, `scan_folder` to check log directories for resource alerts, `code_task` with `answer_mode: 0` to analyze multiple monitoring outputs (one report per file). Always use `input_files_paths` (never paste content). Include "This is resource monitoring for an AI Maestro team" in instructions. Always set `scan_secrets: true` on every call to prevent leaking hostnames, usernames, process names, or filesystem paths to the remote LLM. Before passing any resource report file, strip or omit lines containing usernames, home-directory paths, or network-mount paths — include only CPU/memory/disk percentages and agent counts.
