...
# ── commit-msg hook (governance self-id trailer) ────────────────────────────
# Stamps `Agent: ai-maestro-chief-of-staff` (the plugin slug) onto every commit
# in this repo as a git trailer, so `git blame`/`git log` can attribute a commit
# to the plugin whose development produced it (commit-discipline + PRRD G1.1).
# Two invariants make this safe to run on EVERY commit:
#   • FAIL-OPEN — any error still exits 0. A commit-msg hook that rejects would
#     BLOCK the commit; the trailer is advisory metadata, never a gate.
#   • IDEMPOTENT — `git interpret-trailers --if-exists doNothing` leaves an
#     existing Agent trailer untouched, so re-running (amend, rebase) never
#     double-stamps and an explicit hand-written Agent trailer is respected.
COMMIT_MSG_HOOK_TEMPLATE = r"""#!/usr/bin/env bash
...
def ensure_commit_msg_hook(git_root: Path) -> None:
    """Install / refresh the commit-msg hook (governance self-id trailer).

    Mirrors ``ensure_pre_push_hook``: writes ``.githooks/commit-msg`` from the
    inline template (idempotent — only when content differs) and chmods it.
    ``core.hooksPath`` is already activated by ``ensure_pre_push_hook``, which
    runs just before this in Step 0.5, so the hook is live for every commit in
    this repo. Fail-open by design: it never rejects a commit, it only stamps
    the ``Agent:`` trailer when absent.
    """
    hooks_dir = git_root / ".githooks"
    hooks_dir.mkdir(exist_ok=True)
    hook_path = hooks_dir / "commit-msg"

    if not hook_path.exists() or hook_path.read_text(encoding="utf-8") != COMMIT_MSG_HOOK_TEMPLATE:
        hook_path.write_text(COMMIT_MSG_HOOK_TEMPLATE, encoding="utf-8")
    hook_path.chmod(0o755)
    print(f"{GREEN}ok commit-msg hook installed (Agent trailer){NC}")
...
Examples:
  ...
  %(prog)s --patch --dry-run    # run every validation step fully, then
    ...
    # ── Step 0.5: Install / refresh the git hooks ──
    # pre-push refuses any future `git push` unless publish.py is the caller;
    # commit-msg stamps the `Agent:` governance self-id trailer. Both are
    # regenerated from their inline templates on every run so local edits can't
    # survive as a bypass. ensure_pre_push_hook activates core.hooksPath, which
    # the commit-msg hook rides — so it must run first.
    print(f"\n{BLUE}=== Step 0.5: Install git hooks (pre-push gate + commit-msg trailer) ==={NC}")
    ensure_pre_push_hook(git_root)
    ensure_commit_msg_hook(git_root)
    ...
    for name in (
        ...
        "package.json",
        "Cargo.toml",
        # uv.lock is regenerated by _sync_uv_lock() after the pyproject bump so
